...
const createFocusTrap = function (elements, userOptions) {
	...
	const containersContain = function (element) {
		return !!(element && state.containers.some((container) => container.contains(element)));
	};
	...
	const checkTab = function (e) {
		...
		if (state.tabbableGroups.length > 0) {
			...
			const containerIndex = findIndex(state.tabbableGroups, ({ container }) => container.contains(target));
	...
	trap = {
		...
		deactivate(deactivateOptions) {
			...
			const checkCanReturnFocus = getOption(deactivateOptions, 'checkCanReturnFocus');

			if (onDeactivate) {
				onDeactivate();
			}

			const returnFocus = getOption(deactivateOptions, 'returnFocus', 'returnFocusOnDeactivate');
